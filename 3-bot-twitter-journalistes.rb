...
# pour test envoi direct message "@tfcdg1","@GGalle8",


def send_direct_message
	i = 0
	#binding.pry
	while i < @list_journalist.length
		@client.create_direct_message(@list_journalist[i], "Je suis élève à The Hacking Projet une formation gratuite au code et je viens de faire un bot Twitter trop cool !")		
		i += 1
	end
...
def post_tweet_at_list
	i = 0
	#binding.pry
	while i < @list_journalist.length
		@client.update("#{@list_journalist[i]} Vous n'avez pas entendu parlé de la formation The Hacking Project ? \u{1F632}  THP = apprendre à \u{1F4BB} coder en \u{231B} 12 semaines et \u{1F4B8} gratuitement !! Par ici : https://www.thehackingproject.org/ \u{1F575}")
		i += 1
	end
end
